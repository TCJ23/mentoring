...
public class ReadSAPxlsx {
    static final String SAP_FILE = "./Sample_SAP_DevMan_20180821.xlsx";
     ...
     ArrayList<SAPmodel> readXLSX(String inputFile) throws IOException, InvalidFormatException {
    ...
    private  SAPmodel createSAPmodelFromRow(@NotNull Row row) {
        Cell firstname = row.getCell(0);
        Cell lastname = row.getCell(1);
        SAPmodel saper = new SAPmodel();
//        if (firstname != null && lastname != null && !(firstname.getCellTypeEnum() == CellType.BLANK) && !(lastname.getCellTypeEnum() == CellType.BLANK)) {
